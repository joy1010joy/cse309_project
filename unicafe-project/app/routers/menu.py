...
from app.dependencies import get_admin_user, get_current_user, get_menu_service
...
def public_list(
    ...
    items = service.list_items(include_unavailable=include_unavailable)
    if category:
        items = [item for item in items if item.get("category") == category]
    return [_to_response(i) for i in items]


@public_router.get("/{item_id}", response_model=MenuItemResponse)
def public_get(
    item_id: str,
    service: MenuService = Depends(get_menu_service),
) -> MenuItemResponse:
...
@admin_router.post("", response_model=MenuItemResponse, status_code=201)
def admin_create(
